    ...
    def _setup_chains(self):
        ...
        finding_schemas = [
            ...
            ResponseSchema(
                name="attribute_type",
                description=(
                    "The specific clinical dimension this finding measures, e.g. "
                    "'inspiratory_volume', 'structural_clarity', 'opacity', 'size', "
                    "'vascularity', 'effusion', 'pneumothorax_presence', 'position', "
                    "'count'. Two findings about the same anatomical structure can only "
                    "be considered contradictory later if they share the same attribute_type."
                ),
                type="array",
            ),
        ]
        ...
        self.finding_extractor = (
            ...
            4. The attribute_type — the specific clinical dimension being measured (e.g.
               inspiratory_volume, structural_clarity, opacity, size, vascularity, effusion,
               pneumothorax_presence, position, count). Two findings can only be compared as
               contradictory later if they share the same attribute_type, so be precise and
               consistent about which dimension each finding is actually describing.

            IMPORTANT: The four output arrays (findings, anatomical_targets, clinical_status,
            attribute_type) MUST all be the same length and index-aligned — element i of each
            array must describe the same finding.
        ...
        self.contradiction_detector = (
            ...
            Each finding below is tagged with an attribute_type — the specific clinical dimension
            it measures (e.g. inspiratory_volume, structural_clarity, opacity, size, vascularity,
            effusion, pneumothorax_presence, position, count).

            HARD CONSTRAINT: A contradiction between two DIFFERENT findings (type 1 below) requires
            BOTH (a) the same anatomical_target AND (b) the same attribute_type, with opposing
            values. If two findings share an anatomical_target but have DIFFERENT attribute_types,
            they CANNOT be a type-1 contradiction, no matter how their wording sounds (e.g. "low
            inspiratory_volume" and "well-expanded structural_clarity" both concern the lungs but
            are different attributes, so they never contradict each other on that basis alone).

            A CONTRADICTION occurs when:
            1. Two DIFFERENT findings about the SAME anatomical structure AND the SAME attribute_type make opposite claims (e.g. 'heart is normal size' AND 'severe cardiomegaly', both attribute_type=size). Encode as [index_a, index_b] with two distinct indices.
            ...
            6. Findings that share an anatomical_target but have DIFFERENT attribute_type values
               (e.g. inspiratory_volume vs structural_clarity, or size vs vascularity). Compare
               values only within the same attribute_type.
    ...
    def evaluate_medical_report(self, report: str) -> Dict[str, Any]:
        ...
        for i, target in enumerate(anatomical_targets):
            ...
            finding_entity = MedicalEntity(
                name=finding_name,
                description=f"{findings[i]} [attribute_type={attribute_types[i]}]",
            ...
            relation = MedicalRelation(
                ...
                confidence=1.0,
                evidence=f"Status: {clinical_statuses[i]} | Attribute: {attribute_types[i]}",
                sources=["Report Parser"],
            )
        ...
        # Post-hoc guard: even if the LLM ignores the attribute_type constraint,
        # drop any two-finding (non-self) pair whose findings have different
        # attribute_type values — they cannot be a valid type-1 contradiction.
        filtered_contradictions = []
        filtered_pairs = []
        filtered_severities = []
        n_guard = min(len(contradictions), len(contradiction_pairs))
        for i in range(n_guard):
            a, b = contradiction_pairs[i]
            if a != b and attribute_types[a] != attribute_types[b]:
                continue  # different attribute dimensions -> not a real contradiction
            filtered_contradictions.append(contradictions[i])
            filtered_pairs.append(contradiction_pairs[i])
            filtered_severities.append(
                severity_scores[i] if i < len(severity_scores) else 0.5
            )
        contradictions = filtered_contradictions
        contradiction_pairs = filtered_pairs
        severity_scores = filtered_severities
...
def main():
    ...
    result_1 = system.evaluate_medical_report(sample_report)
    ...
    result_2 = system.evaluate_medical_report(hallucinated_report)
